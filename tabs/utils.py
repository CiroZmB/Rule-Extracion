...
# ═══════════════════════════════════════════════════════════════
# FUNCIÓN PARA GENERAR REGLAS CON ONER
# ═══════════════════════════════════════════════════════════════
def crear_reglas_oner(variable, target, n_bins=5, nombre_variable=None):
    """
    Función súper simple para crear reglas OneR

    Parameters:
    -----------
    variable : array-like o DataFrame, la variable a partir en bins
    target : array-like, el target continuo
    n_bins : int, número de bins
    nombre_variable : str, nombre de la variable (opcional)

    Returns:
    --------
    DataFrame con columnas: 'regla' y 'rendimiento'
    """

    # Convertir DataFrame a Series si es necesario
    if isinstance(variable, pd.DataFrame):
        if variable.shape[1] == 1:
            variable_values = variable.iloc[:, 0]
            if nombre_variable is None:
                nombre_variable = variable.columns[0]
        else:
            raise ValueError("Si pasas un DataFrame, debe tener solo 1 columna")
    elif isinstance(variable, pd.Series):
        variable_values = variable
        if nombre_variable is None:
            nombre_variable = variable.name if variable.name else 'variable'
    else:
        variable_values = variable
        if nombre_variable is None:
            nombre_variable = 'variable'

    # Partir en bins con igual número de muestras (quantiles)
    try:
        binned_series = pd.qcut(variable_values, q=n_bins, duplicates='drop')
        bins = binned_series.cat.categories
    except:
        # Si no se puede partir, usar valores únicos
        unique_vals = np.unique(variable_values)
        bins = [f"== {val}" for val in unique_vals]
        binned_series = pd.Series([f"== {val}" for val in variable_values])

    # Crear DataFrame para facilitar agrupación
    df = pd.DataFrame({
    ...
    # Calcular suma por bin
    resultado = df.groupby('bin')['target'].sum().reset_index()

    # Crear reglas en formato query de pandas
    reglas = []
    for bin_interval in resultado['bin']:
        if hasattr(bin_interval, 'left') and hasattr(bin_interval, 'right'):
            # Es un intervalo de pd.cut
            left = bin_interval.left
            right = bin_interval.right

            if bin_interval.closed == 'right':
                regla = f"{nombre_variable} > {left} and {nombre_variable} <= {right}"
            elif bin_interval.closed == 'left':
                regla = f"{nombre_variable} >= {left} and {nombre_variable} < {right}"
            else:
                regla = f"{nombre_variable} > {left} and {nombre_variable} < {right}"
        else:
            # Es un valor categórico
            regla = f"{nombre_variable} {bin_interval}"
    ...
    df_reglas = pd.DataFrame({
        'regla': reglas,
        'rendimiento': resultado['target'].values
    })

    return df_reglas

# ═══════════════════════════════════════════════════════════════
# FUNCIÓN PARA EXTRAER MEJORES REGLAS LONG/SHORT
# ═══════════════════════════════════════════════════════════════
def extraer_mejores_reglas(train_df, test_df, selected_features, n_bins=5):
    """
    Extrae las mejores reglas LONG y SHORT para cada variable seleccionada
    
    Parameters:
    -----------
    train_df : pandas.DataFrame
        DataFrame de entrenamiento con features y Target
    test_df : pandas.DataFrame  
        DataFrame de test con features y Target
    selected_features : list
        Lista de nombres de features seleccionadas
    n_bins : int
        Número de bins para OneR
        
    Returns:
    --------
    pandas.DataFrame
        DataFrame con columnas: variable, tipo, regla, rendimiento_train, rendimiento_test
    """
    
    # Validaciones
    if 'Target' not in train_df.columns or 'Target' not in test_df.columns:
        raise ValueError("Los DataFrames deben contener la columna 'Target'")
    ...
    for variable in selected_features:
        if variable not in train_df.columns:
            print(f"Warning: Variable '{variable}' no encontrada en train_df")
            continue
            
        try:
            # Eliminar NaN en train para esta variable
            train_clean = train_df[[variable, 'Target']].dropna()
            
            if len(train_clean) < n_bins * 2:  # Mínimo para crear bins
                print(f"Warning: Datos insuficientes para '{variable}' en train")
                continue
            
            # Generar reglas OneR para esta variable
            reglas_df = crear_reglas_oner(
                train_clean[variable], 
                train_clean['Target'], 
                n_bins=n_bins, 
                nombre_variable=variable
            )
            ...
            if len(reglas_df) == 0:
                print(f"Warning: No se generaron reglas para '{variable}'")
                continue
            
            # Encontrar mejor regla LONG (mayor rendimiento)
            idx_long = reglas_df['rendimiento'].idxmax()
            regla_long = reglas_df.loc[idx_long]
            
            # Encontrar mejor regla SHORT (menor rendimiento)
            idx_short = reglas_df['rendimiento'].idxmin()
            regla_short = reglas_df.loc[idx_short]
            
            # Evaluar reglas en test
            rendimiento_test_long = evaluar_regla_en_test(test_df, regla_long['regla'])
            rendimiento_test_short = evaluar_regla_en_test(test_df, regla_short['regla'])
            
            # Agregar resultados LONG
            resultados.append({
                'variable': variable,
                'tipo': 'LONG',
                'regla': regla_long['regla'],
                'rendimiento_train': regla_long['rendimiento'],
                'rendimiento_test': rendimiento_test_long
            })
            
            # Agregar resultados SHORT (invertir signos para representar ganancia con SHORT)
            resultados.append({
                'variable': variable,
                'tipo': 'SHORT',
                'regla': regla_short['regla'],
                'rendimiento_train': -regla_short['rendimiento'],  # Invertir: ganamos cuando el mercado baja
                'rendimiento_test': -rendimiento_test_short        # Invertir: ganamos cuando el mercado baja
            })
            
        except Exception as e:
            print(f"Error procesando variable '{variable}': {str(e)}")
            continue
...
# ═══════════════════════════════════════════════════════════════
# FUNCIÓN AUXILIAR PARA EVALUAR REGLAS EN TEST
# ═══════════════════════════════════════════════════════════════
def evaluar_regla_en_test(test_df, regla):
    """
    Evalúa una regla en el conjunto de test y devuelve la suma del target
    
    Parameters:
    -----------
    test_df : pandas.DataFrame
        DataFrame de test con Target
    regla : str
        Regla en formato query de pandas
        
    Returns:
    --------
    float
        Suma del target donde la regla se cumple, 0 si no hay matches o error
    """
    ...
    try:
        ...
        # Aplicar regla usando query
        matches = test_clean.query(regla)
        
        if len(matches) == 0:
            return 0.0
        
        # Devolver suma del target
        return matches['Target'].sum()
        
    except Exception as e:
        print(f"Error evaluando regla '{regla}': {str(e)}")
        return 0.0

# ═══════════════════════════════════════════════════════════════
# FUNCIÓN PARA ANÁLIZAR PERFORMANCE DE REGLAS
# ═══════════════════════════════════════════════════════════════
def analizar_performance_reglas(reglas_df):
    """
    Analiza la performance de las reglas extraídas
    
    Parameters:
    -----------
    reglas_df : pandas.DataFrame
        DataFrame resultado de extraer_mejores_reglas
    ...
    if len(reglas_df) == 0:
        return {"error": "No hay reglas para analizar"}
    
    # Separar LONG y SHORT
    long_rules = reglas_df[reglas_df['tipo'] == 'LONG']
    short_rules = reglas_df[reglas_df['tipo'] == 'SHORT']
    
    stats = {
        'total_reglas': len(reglas_df),
        'reglas_long': len(long_rules),
        'reglas_short': len(short_rules),
        
        # Performance en TRAIN
        'train_rendimiento_promedio': reglas_df['rendimiento_train'].mean(),
        'train_rendimiento_long_promedio': long_rules['rendimiento_train'].mean() if len(long_rules) > 0 else 0,
        'train_rendimiento_short_promedio': short_rules['rendimiento_train'].mean() if len(short_rules) > 0 else 0,
        
        # Performance en TEST
        'test_rendimiento_promedio': reglas_df['rendimiento_test'].mean(),
        'test_rendimiento_long_promedio': long_rules['rendimiento_test'].mean() if len(long_rules) > 0 else 0,
        'test_rendimiento_short_promedio': short_rules['rendimiento_test'].mean() if len(short_rules) > 0 else 0,
        
        # Consistencia (correlación train vs test)
        'consistencia_general': reglas_df['rendimiento_train'].corr(reglas_df['rendimiento_test']),
        
        # Mejores reglas
        'mejor_regla_train': reglas_df.loc[reglas_df['rendimiento_train'].abs().idxmax()] if len(reglas_df) > 0 else None,
        'mejor_regla_test': reglas_df.loc[reglas_df['rendimiento_test'].abs().idxmax()] if len(reglas_df) > 0 else None,
    }
    
    return stats

# ═══════════════════════════════════════════════════════════════
# FUNCIONES PARA VALIDACIÓN CON MONKEY TEST
# ═══════════════════════════════════════════════════════════════

def monkey_test_trading(df_oos, tipo_regla='LONG', fraction=1/3, n_simulations=1000, quantile=80):
    """
    Ejecuta simulaciones Monte Carlo para establecer baseline de trading aleatorio
    
    Parameters:
    -----------
    df_oos : pandas.DataFrame
        DataFrame del período OOS/test con columna 'Target'
    tipo_regla : str
        'LONG' o 'SHORT' - determina la dirección de la simulación
    fraction : float
        Fracción del dataset a usar en cada simulación (default: 1/3)
    n_simulations : int
        Número de simulaciones
    quantile : float
        Quantil para establecer el umbral (0-100)
        
    Returns:
    --------
    dict : Estadísticas de las simulaciones
    """
    ...
    if tipo_regla not in ['LONG', 'SHORT']:
        raise ValueError("tipo_regla debe ser 'LONG' o 'SHORT'")
    
    # Limpiar datos
    df_clean = df_oos.dropna(subset=['Target'])
    
    if len(df_clean) == 0:
        raise ValueError("No hay datos válidos en el DataFrame OOS")
    
    # Calcular número de muestras por simulación
    n_samples = int(len(df_clean) * fraction)
    
    if n_samples < 1:
        n_samples = 1
    
    print(f"🔄 Ejecutando {n_simulations:,} simulaciones {tipo_regla} con {n_samples} muestras cada una ({fraction:.1%} del dataset)...")
    
    # Realizar simulaciones
    cum_returns_list = []
    
    for n in range(n_simulations):
        muestra_aleatoria = df_clean.sample(n=n_samples)['Target']
        
        if tipo_regla == 'SHORT':
            # Para SHORT: ganamos cuando el mercado baja (inverso)
            cum_returns = (-1 * muestra_aleatoria).sum()
        else:  # LONG
            # Para LONG: ganamos cuando el mercado sube (directo)
            cum_returns = (1 * muestra_aleatoria).sum()
        
        cum_returns_list.append(cum_returns)
    
    cum_returns_array = np.array(cum_returns_list)
    
    # Calcular estadísticas
    stats = {
        'tipo_regla': tipo_regla,
        'mean': np.mean(cum_returns_array),
        'median': np.median(cum_returns_array),
        'std': np.std(cum_returns_array),
        'min': np.min(cum_returns_array),
        'max': np.max(cum_returns_array),
        'quantile_value': np.percentile(cum_returns_array, quantile),
        'quantile_percent': quantile,
        'data': cum_returns_array,
        'n_samples_per_sim': n_samples,
        'total_oos_size': len(df_clean),
...
def create_monkey_test_chart(stats_monkey, titulo_personalizado=None):
    """
    Crea un gráfico de histograma elegante para el Monkey Test usando Plotly
    
    Parameters:
    -----------
    stats_monkey : dict
        Estadísticas del monkey test (resultado de monkey_test_trading)
    ...
    data = stats_monkey['data']
    tipo_regla = stats_monkey['tipo_regla']
    
    # Colores según tipo de regla
    colors = {
        'LONG': {'hist': '#2E8B57', 'quantile': '#FF6347', 'mean': '#4169E1', 'median': '#9932CC'},
        'SHORT': {'hist': '#DC143C', 'quantile': '#FF8C00', 'mean': '#4169E1', 'median': '#9932CC'}
    }
    
    color_scheme = colors[tipo_regla]
    ...
    fig.add_trace(go.Histogram(
        x=data,
        nbinsx=max(30, int(np.sqrt(len(data)))),
        name=f'Simulaciones {tipo_regla}',
        marker_color=color_scheme['hist'],
        opacity=0.7,
    ...
    # Línea de quantil
    fig.add_vline(
        x=stats_monkey['quantile_value'],
        line_dash="dash",
        line_color=color_scheme['quantile'],
        line_width=3,
        annotation_text=f"Quantil {stats_monkey['quantile_percent']}%: {stats_monkey['quantile_value']:.4f}",
    ...
    fig.add_vline(
        x=stats_monkey['mean'],
        line_dash="solid",
        line_color=color_scheme['mean'],
        line_width=2,
        annotation_text=f"Media: {stats_monkey['mean']:.4f}",
        annotation_position="top left"
    )
    
    # Línea de mediana
    fig.add_vline(
        x=stats_monkey['median'],
        line_dash="dot",
        line_color=color_scheme['median'],
        line_width=2,
        annotation_text=f"Mediana: {stats_monkey['median']:.4f}",
    ...
    if titulo_personalizado:
        ...
        titulo = (f'Monkey Test - Simulaciones {tipo_regla}<br>'
                 f'<sub>{stats_monkey["n_simulations"]:,} simulaciones × {stats_monkey["n_samples_per_sim"]} muestras '
                 f'({stats_monkey["fraction_used"]:.1%} del dataset)</sub>')
    ...
    fig.update_layout(
        ...
        annotations=[
            dict(
                ...
                text=(f'<b>Estadísticas {tipo_regla}:</b><br>'
                     f'Dataset: {stats_monkey["total_oos_size"]} obs<br>'
                     f'Muestra/sim: {stats_monkey["n_samples_per_sim"]} ({stats_monkey["fraction_used"]:.1%})<br>'
                     f'Media: {stats_monkey["mean"]:.4f}<br>'
                     f'Mediana: {stats_monkey["median"]:.4f}<br>'
                     f'Desv. Std: {stats_monkey["std"]:.4f}<br>'
                     f'Quantil {stats_monkey["quantile_percent"]}%: {stats_monkey["quantile_value"]:.4f}<br>'
                     f'Rango: [{stats_monkey["min"]:.4f}, {stats_monkey["max"]:.4f}]'),
                showarrow=False,
                align='left',
...
# ═══════════════════════════════════════════════════════════════
# FUNCIONES PARA COMBINACIONES DE REGLAS
# ═══════════════════════════════════════════════════════════════

def generar_combinaciones_reglas(reglas_validadas):
    """
    Genera todas las combinaciones de 2 reglas del mismo tipo
    
    Parameters:
    -----------
    reglas_validadas : pandas.DataFrame
        DataFrame de reglas validadas con columnas: variable, tipo, regla, etc.
        
    Returns:
    --------
    list
        Lista de diccionarios con las combinaciones generadas
    """
    ...
    # Separar por tipo
    long_rules = reglas_validadas[reglas_validadas['tipo'] == 'LONG']
    short_rules = reglas_validadas[reglas_validadas['tipo'] == 'SHORT']
    
    # Generar combinaciones LONG (2 reglas)
    if len(long_rules) >= 2:
        for combo in combinations(long_rules.iterrows(), 2):
            idx1, regla1 = combo[0]
            idx2, regla2 = combo[1]
            
            combinaciones.append({
                'tipo': 'LONG',
                'variable1': regla1['variable'],
                'variable2': regla2['variable'],
                'regla1': regla1['regla'],
                'regla2': regla2['regla'],
                'regla_compuesta': f"({regla1['regla']}) and ({regla2['regla']})",  # ← CAMBIADO: AND → and
                'descripcion': f"{regla1['variable']} + {regla2['variable']}"
            })
    
    # Generar combinaciones SHORT (2 reglas)
    if len(short_rules) >= 2:
        for combo in combinations(short_rules.iterrows(), 2):
            idx1, regla1 = combo[0]
            idx2, regla2 = combo[1]
...
def evaluar_combinacion_reglas(df_completo, regla1, regla2, tipo_regla):
    """
    Evalúa una combinación de 2 reglas con lógica AND (VERSIÓN OPTIMIZADA)
    
    Parameters:
    -----------
    df_completo : pandas.DataFrame
        DataFrame completo (train + test) con todas las columnas
    regla1 : str
        Primera regla en formato query de pandas
    regla2 : str
        Segunda regla en formato query de pandas
    tipo_regla : str
        'LONG' o 'SHORT'
    ...
    --------
    dict
        Diccionario con métricas de la combinación
    """
    
    try:
        # Limpiar datos una sola vez
        df_clean = df_completo.dropna(subset=['Target'])
        ...
        # OPTIMIZACIÓN: Combinar ambas reglas en una sola query
        try:
            regla_combinada = f"({regla1}) and ({regla2})"
            matches_combinacion = df_clean.query(regla_combinada)
            
        except Exception as e:
            return {
                'rendimiento_total': 0.0,
                'operaciones': 0,
                'error': f'Error aplicando reglas: {str(e)}'
            }
        
        # Calcular métricas
        operaciones = len(matches_combinacion)
        
        if operaciones == 0:
            rendimiento_total = 0.0
        else:
            # Calcular rendimiento según tipo
            if tipo_regla == 'LONG':
                # Para LONG: rendimiento directo
                rendimiento_total = matches_combinacion['Target'].sum()
            else:  # SHORT
                # Para SHORT: rendimiento invertido (ganamos cuando el mercado baja)
                rendimiento_total = (-1 * matches_combinacion['Target']).sum()
...
def calcular_metricas_combinaciones(df_completo, combinaciones, progress_callback=None):
    """
    Calcula métricas para todas las combinaciones de reglas (VERSIÓN OPTIMIZADA CON PROGRESO)
    
    Parameters:
    -----------
    df_completo : pandas.DataFrame
        DataFrame completo (train + test)
    combinaciones : list
        Lista de combinaciones generadas por generar_combinaciones_reglas
    progress_callback : function, optional
        Función callback para actualizar progreso (recibe porcentaje 0-100)
        
    Returns:
    --------
    pandas.DataFrame
        DataFrame con métricas de todas las combinaciones
    """
    
    print(f"🔄 Evaluando {len(combinaciones)} combinaciones de reglas...")
    ...
    for i, combo in enumerate(combinaciones):
        ...
        if progress_callback:
            progreso = int((i + 1) / total_combinaciones * 100)
            progress_callback(progreso)
        
        # Mostrar progreso en consola cada 25%
        if total_combinaciones >= 4:
            quarter_points = [total_combinaciones // 4, total_combinaciones // 2, 
                            3 * total_combinaciones // 4, total_combinaciones]
            if (i + 1) in quarter_points:
                porcentaje = int((i + 1) / total_combinaciones * 100)
                print(f"   Progreso: {porcentaje}% ({i + 1}/{total_combinaciones} combinaciones)")
        
        # Evaluar combinación (OPTIMIZADA)
        metricas = evaluar_combinacion_reglas(
            df_completo,
            combo['regla1'],
            combo['regla2'],
            combo['tipo']
        ...
        resultado = {
            ...
            'tipo': combo['tipo'],
            'variable1': combo['variable1'],
            'variable2': combo['variable2'],
            'regla_compuesta': combo['regla_compuesta'],
            'rendimiento_total': metricas['rendimiento_total'],
            'operaciones': metricas['operaciones'],
            'rendimiento_por_operacion': (
                metricas['rendimiento_total'] / metricas['operaciones'] 
                if metricas['operaciones'] > 0 else 0.0
            ),
    ...
    df_resultados = df_resultados.sort_values('rendimiento_total', ascending=False)
    
    # Reset index
    df_resultados = df_resultados.reset_index(drop=True)
    
    print(f"✅ Evaluación completada: {len(df_resultados)} combinaciones procesadas")
    
    return df_resultados

def analizar_combinaciones_reglas(df_combinaciones):
    """
    Analiza estadísticas de las combinaciones de reglas
    
    Parameters:
    -----------
    df_combinaciones : pandas.DataFrame
        DataFrame resultado de calcular_metricas_combinaciones
        
    Returns:
    --------
    dict
        Estadísticas de las combinaciones
    """
    
    if len(df_combinaciones) == 0:
        return {"error": "No hay combinaciones para analizar"}
    
    # Filtrar combinaciones sin errores
    df_validas = df_combinaciones[df_combinaciones['error'].isna()].copy()
    
    if len(df_validas) == 0:
        return {"error": "No hay combinaciones válidas"}
    
    # Separar por tipo
    long_combos = df_validas[df_validas['tipo'] == 'LONG']
    short_combos = df_validas[df_validas['tipo'] == 'SHORT']
    
    # Filtrar combinaciones con rendimiento positivo
    combos_positivas = df_validas[df_validas['rendimiento_total'] > 0]
    long_positivas = long_combos[long_combos['rendimiento_total'] > 0]
    short_positivas = short_combos[short_combos['rendimiento_total'] > 0]
    
    stats = {
        'total_combinaciones': len(df_combinaciones),
        'combinaciones_validas': len(df_validas),
        'combinaciones_con_errores': len(df_combinaciones) - len(df_validas),
        
        # Por tipo
        'combinaciones_long': len(long_combos),
        'combinaciones_short': len(short_combos),
        
        # Rendimientos positivos
        'combinaciones_positivas': len(combos_positivas),
        'long_positivas': len(long_positivas),
        'short_positivas': len(short_positivas),
        'tasa_exito': len(combos_positivas) / len(df_validas) if len(df_validas) > 0 else 0,
        
        # Estadísticas de rendimiento
        'rendimiento_promedio': df_validas['rendimiento_total'].mean(),
        'rendimiento_mediano': df_validas['rendimiento_total'].median(),
        'rendimiento_maximo': df_validas['rendimiento_total'].max(),
        'rendimiento_minimo': df_validas['rendimiento_total'].min(),
        
        # Estadísticas de operaciones
        'operaciones_promedio': df_validas['operaciones'].mean(),
        'operaciones_mediana': df_validas['operaciones'].median(),
        'operaciones_maximo': df_validas['operaciones'].max(),
        'operaciones_minimo': df_validas['operaciones'].min(),
        
        # Mejores combinaciones
        'mejor_combinacion': df_validas.loc[df_validas['rendimiento_total'].idxmax()] if len(df_validas) > 0 else None,
        'mas_operaciones': df_validas.loc[df_validas['operaciones'].idxmax()] if len(df_validas) > 0 else None,
        'mejor_ratio': df_validas.loc[df_validas['rendimiento_por_operacion'].idxmax()] if len(df_validas) > 0 else None
    }
    
    return stats

# ═══════════════════════════════════════════════════════════════
# FUNCIONES PARA ANÁLISIS DE ESTABILIDAD TEMPORAL (ENSEMBLE)
# ═══════════════════════════════════════════════════════════════

def dividir_historico_en_trozos(df_completo, n_trozos):
    """
    Divide el histórico en N trozos iguales por tamaño
    
    Parameters:
    -----------
    df_completo : pandas.DataFrame
        DataFrame completo con índice de fechas y columna 'Target'
    n_trozos : int
        Número de trozos en los que dividir el histórico
        
    Returns:
    --------
    list
        Lista de DataFrames, uno por cada trozo
    """
    
    # Limpiar datos y ordenar por fecha
    df_clean = df_completo.dropna(subset=['Target']).sort_index()
    
    if len(df_clean) == 0:
        raise ValueError("No hay datos válidos para dividir")
    
    total_rows = len(df_clean)
    rows_per_trozo = total_rows // n_trozos
    
    if rows_per_trozo == 0:
        raise ValueError(f"No hay suficientes datos para dividir en {n_trozos} trozos")
    
    trozos = []
    
    for i in range(n_trozos):
        start_idx = i * rows_per_trozo
        
        if i == n_trozos - 1:  # Último trozo incluye el resto
            end_idx = total_rows
        else:
            end_idx = (i + 1) * rows_per_trozo
        
        trozo = df_clean.iloc[start_idx:end_idx].copy()
        trozos.append(trozo)
    
    return trozos

def evaluar_regla_en_trozo(df_trozo, regla_compuesta, tipo_regla):
    """
    Evalúa una regla en un trozo específico del histórico
    
    Parameters:
    -----------
    df_trozo : pandas.DataFrame
        DataFrame de un trozo del histórico
    regla_compuesta : str
        Regla en formato query de pandas
    tipo_regla : str
        'LONG' o 'SHORT'
    ...
    --------
    float
        Rendimiento de la regla en este trozo
    """
    
    try:
        # Aplicar regla: M * historico.query(regla)['Target'].sum()
        matches = df_trozo.query(regla_compuesta)
        
        if len(matches) == 0:
            return 0.0
        
        # Definir multiplicador M según tipo
        M = 1 if tipo_regla == 'LONG' else -1
        
        # Calcular rendimiento: M * suma de targets
        rendimiento = M * matches['Target'].sum()
...
def calcular_estabilidad_reglas(df_completo, reglas_validadas, n_trozos):
    """
    Calcula la estabilidad temporal de las reglas validadas (VERSIÓN MEJORADA)
    ...
    df_completo : pandas.DataFrame
        DataFrame completo (train + test)
    reglas_validadas : pandas.DataFrame
        DataFrame de reglas validadas del tab VALIDATION
    n_trozos : int
        Número de trozos para dividir el histórico
        
    Returns:
    --------
    pandas.DataFrame
        DataFrame con análisis de estabilidad por regla
    """
    
    print(f"🔄 Analizando estabilidad de {len(reglas_validadas)} reglas en {n_trozos} trozos...")
    
    # Dividir histórico en trozos
    trozos = dividir_historico_en_trozos(df_completo, n_trozos)
    
    print(f"📊 Histórico dividido en {len(trozos)} trozos:")
    for i, trozo in enumerate(trozos):
        print(f"   Trozo {i+1}: {len(trozo)} observaciones ({trozo.index.min()} → {trozo.index.max()})")
    
    resultados = []
    
    for idx, (_, regla_info) in enumerate(reglas_validadas.iterrows()):
        
        print(f"\n🔍 Evaluando regla {idx+1}/{len(reglas_validadas)}: {regla_info['descripcion']} ({regla_info['tipo']})")
        
        # Evaluar regla en cada trozo
        rendimientos_por_trozo = []
        
        for i, trozo in enumerate(trozos):
            rendimiento = evaluar_regla_en_trozo(
                trozo, 
                regla_info['regla_compuesta'], 
                regla_info['tipo']
            )
            rendimientos_por_trozo.append(rendimiento)
            print(f"   Trozo {i+1}: {rendimiento:.6f}")
        
        # Calcular métricas de estabilidad
        rendimientos_array = np.array(rendimientos_por_trozo)
        
        # Rendimiento promedio
        rendimiento_promedio = np.mean(rendimientos_array)
        
        # Desviación estándar de rendimientos
        std_rendimientos = np.std(rendimientos_array, ddof=0)  # Usar ddof=0 para población
        
        # Ratio de estabilidad (promedio / std)
        if std_rendimientos > 0:
            ratio_estabilidad = rendimiento_promedio / std_rendimientos
        else:
            # Si std es 0, significa rendimiento constante
            # Asignar valor alto si rendimiento es positivo, bajo si es negativo/cero
            if rendimiento_promedio > 0:
                ratio_estabilidad = float('inf')
            elif rendimiento_promedio < 0:
                ratio_estabilidad = float('-inf')
            else:
                ratio_estabilidad = 0.0
        
        print(f"   📊 Promedio: {rendimiento_promedio:.6f}, Std: {std_rendimientos:.6f}, Ratio: {ratio_estabilidad:.3f}")
        
        # Agregar resultado
        resultado_regla = {
            'descripcion': regla_info['descripcion'],
            'tipo': regla_info['tipo'],
            'regla_compuesta': regla_info['regla_compuesta'],
            
            # Métricas originales
            'rendimiento_total_original': regla_info['rendimiento_total'],
            'operaciones_total_original': regla_info['operaciones'],
            
            # Métricas de estabilidad
            'rendimiento_promedio': rendimiento_promedio,
            'std_rendimientos': std_rendimientos,
            'ratio_estabilidad': ratio_estabilidad,
            
            # Detalles por trozo
            'rendimientos_por_trozo': rendimientos_por_trozo
        }
    ...
    print(f"\n✅ Análisis de estabilidad completado para {len(df_estabilidad)} reglas")
    print(f"📈 Ratios calculados - LONG promedio: {df_estabilidad[df_estabilidad['tipo'] == 'LONG']['ratio_estabilidad'].mean():.3f}")
    print(f"📉 Ratios calculados - SHORT promedio: {df_estabilidad[df_estabilidad['tipo'] == 'SHORT']['ratio_estabilidad'].mean():.3f}")
    
    return df_estabilidad

def seleccionar_mejores_reglas_por_estabilidad(df_estabilidad, n_reglas):
    """
    Selecciona las mejores reglas por ratio de estabilidad
    
    Parameters:
    -----------
    df_estabilidad : pandas.DataFrame
        DataFrame resultado de calcular_estabilidad_reglas
    n_reglas : int
        Número de reglas a seleccionar por tipo (LONG y SHORT)
        
    Returns:
    --------
    tuple
        (top_long_df, top_short_df) - DataFrames con las mejores reglas por tipo
    """
    
    # Separar por tipo y ordenar por ratio de estabilidad (descendente)
    long_rules = df_estabilidad[df_estabilidad['tipo'] == 'LONG'].copy()
    short_rules = df_estabilidad[df_estabilidad['tipo'] == 'SHORT'].copy()
    
    # Ordenar por ratio de estabilidad
    long_rules = long_rules.sort_values('ratio_estabilidad', ascending=False)
    short_rules = short_rules.sort_values('ratio_estabilidad', ascending=False)
    
    # Seleccionar top N
    top_long = long_rules.head(min(n_reglas, len(long_rules)))
    top_short = short_rules.head(min(n_reglas, len(short_rules)))
    
    # Reset index
    top_long = top_long.reset_index(drop=True)
    top_short = top_short.reset_index(drop=True)
        ...
        try:
            ...
            try:
                ...
                if isinstance(contenido_html, bytes):
                    contenido_html = contenido_html.decode('utf-16-le', errors='ignore')
                ...
                patron = r'(\d{4}\.\d{2}\.\d{2})\s*-\s*(\d{4}\.\d{2}\.\d{2})'               
                match = re.search(patron, contenido_html, re.IGNORECASE)
...
def calcular_operaciones_por_año(fecha_inicio, fecha_fin, num_operaciones):
    ...
    try:
        ...
        if operaciones_por_año > num_operaciones:
            operaciones_por_año = num_operaciones
...
def debug_completo_archivo(archivo_html):
    """
    Función de debug completo para ver qué está pasando con tu archivo
    """
    import re
    print("="*70)
    print("DEBUG COMPLETO DEL ARCHIVO HTML")
    print("="*70)
    ...
    try:
        ...
        print(f"1. Tipo de contenido inicial: {type(contenido)}")
        print(f"2. Tamaño del contenido: {len(contenido)} caracteres/bytes")
        
        # Convertir si es necesario
        if isinstance(contenido, bytes):
            print("3. Convirtiendo de bytes a string...")
            contenido = contenido.decode('utf-8', errors='ignore')
        
        print(f"4. Tamaño después de conversión: {len(contenido)} caracteres")
        
        # Mostrar primeros caracteres
        print("\n5. PRIMEROS 300 CARACTERES:")
        print("-" * 50)
        print(repr(contenido[:300]))
        print("-" * 50)
        
        # Buscar líneas que contengan fechas o palabras clave
        print("\n6. LÍNEAS RELEVANTES:")
        print("-" * 50)
        lineas = contenido.split('\n')
        for i, linea in enumerate(lineas):
            linea_clean = linea.strip()
            if any(palabra in linea_clean.lower() for palabra in ['período', 'periodo', 'h4', '2019', '2025']):
                print(f"Línea {i:3d}: {repr(linea_clean)}")
        
        # Buscar TODOS los números que parecen fechas
        print("\n7. BÚSQUEDA DE PATRONES DE FECHA:")
        print("-" * 50)
        
        # Todas las secuencias de 4 dígitos (años)
        años = re.findall(r'\b\d{4}\b', contenido)
        print(f"Años encontrados: {años}")
        
        # Todas las fechas individuales con puntos
        fechas_punto = re.findall(r'\d{4}\.\d{1,2}\.\d{1,2}', contenido)
        print(f"Fechas con puntos: {fechas_punto}")
        
        # Todas las fechas con barras
        fechas_barra = re.findall(r'\d{4}/\d{1,2}/\d{1,2}', contenido)
        print(f"Fechas con barras: {fechas_barra}")
        
        # Todas las fechas con guiones
        fechas_guion = re.findall(r'\d{4}-\d{1,2}-\d{1,2}', contenido)
        print(f"Fechas con guiones: {fechas_guion}")
        
        print("\n8. BÚSQUEDA DE RANGOS DE FECHAS:")
        print("-" * 50)
        
        # Diferentes patrones de rangos
        patrones_rango = [
            (r'(\d{4}\.\d{2}\.\d{2})\s*-\s*(\d{4}\.\d{2}\.\d{2})', 'Patrón estándar (YYYY.MM.DD - YYYY.MM.DD)'),
            (r'(\d{4}\.\d{1,2}\.\d{1,2})\s*-\s*(\d{4}\.\d{1,2}\.\d{1,2})', 'Patrón flexible (días/meses 1-2 dígitos)'),
            (r'(\d{4}\.\d{2}\.\d{2})\s*[-–—]\s*(\d{4}\.\d{2}\.\d{2})', 'Diferentes tipos de guión'),
            (r'(\d{4}/\d{2}/\d{2})\s*-\s*(\d{4}/\d{2}/\d{2})', 'Con barras'),
            (r'(\d{4}-\d{2}-\d{2})\s*-\s*(\d{4}-\d{2}-\d{2})', 'Con guiones ISO'),
        ]
        
        for patron, descripcion in patrones_rango:
            resultado = re.search(patron, contenido)
            if resultado:
                print(f"✅ {descripcion}")
                print(f"   Fecha inicio: {resultado.group(1)}")
                print(f"   Fecha fin: {resultado.group(2)}")
                return resultado.group(1), resultado.group(2)
            else:
                print(f"❌ {descripcion}")
        
        print("\n9. BÚSQUEDA EN CONTEXTO H4:")
        print("-" * 50)
        
        # Buscar todo lo que esté entre paréntesis después de H4
        h4_matches = re.findall(r'H4\s*\([^)]+\)', contenido, re.IGNORECASE)
        for match in h4_matches:
            print(f"Contexto H4 encontrado: {repr(match)}")
        
        print("\n10. ANÁLISIS DE CARACTERES ESPECIALES:")
        print("-" * 50)
        
        # Buscar caracteres que no sean ASCII
        caracteres_especiales = set()
        for char in contenido:
            if ord(char) > 127:
                caracteres_especiales.add(char)
        
        if caracteres_especiales:
            print(f"Caracteres no-ASCII encontrados: {list(caracteres_especiales)}")
        else:
            print("No se encontraron caracteres especiales")
        
        print("\n" + "="*70)
        print("RESULTADO: No se encontraron fechas en formato esperado")
        print("="*70)
...
# Función para usar en tu código
def usar_debug_en_tu_codigo(archivo_html):
    """
    Usar esta función para hacer el debug
    """
    fecha_inicio, fecha_fin = debug_completo_archivo(archivo_html)
    
    if fecha_inicio and fecha_fin:
        print(f"\n🎉 ÉXITO! Fechas extraídas:")
        print(f"fecha_inicio = '{fecha_inicio}'")
        print(f"fecha_fin = '{fecha_fin}'")
    else:
        print(f"\n❌ No se pudieron extraer las fechas")
        print("Revisar el output del debug arriba para identificar el problema")
